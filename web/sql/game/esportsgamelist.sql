...
FROM
    (SELECT DISTINCT game.name, game.game_id
    FROM game INNER JOIN twitch_stream ON game.game_id = twitch_stream.game_id) AS g,
    ...
    (
        ...
        FROM twitch_game_vc
        WHERE epoch >= ((SELECT EXTRACT(epoch FROM now())) - 2592000)
